...
def create_code_workspace(
    ...
    """
    resolved_source = (source or repository_url or "").strip()
    resolved_label = (label or name or "").strip()

    def _do():
        return _get_client().create_code_workspace(
            source=resolved_source or None,
            branch=branch.strip() or None,
            repo_connection_id=repo_connection_id.strip() or None,
            label=resolved_label or None,
            company_id=company_id.strip() or None,
        )

    result = _call_with_retry(_do)
    workspace = result.get("workspace", result) if isinstance(result, dict) else {}
    workspace_id = workspace.get("id") or workspace.get("workspaceRunnerId") or ""
    status = workspace.get("status", "")
    if not workspace_id:
        return json.dumps(result, default=str)[:1000]
    bits = [f"Created code workspace `{workspace_id}`"]
    if resolved_label:
        bits.append(f"({resolved_label})")
    if status:
        bits.append(f"[{status}]")
    bits.append("— use this id with coding_run_command / code_workspace_chat.")
    return " ".join(bits)
...
def memory_search(query: str, namespace: str = "default", top_k: int = 5) -> str:
    ...
    for m in result[:top_k]:
        ...
        value = (m.get("summary") or m.get("value") or "")[:150]
        lines.append(f"- `{key}`: {value}")
    return "\n".join(lines)


# ── Projects (product-machine / consulting) ──────────────────────────
...
def create_project(
    ...
    result = _call_with_retry(_do)
    pid = result.get("id") or result.get("projectId") or ""
    pname = result.get("name") or name
    status = result.get("status", "")
    if not pid:
        return json.dumps(result, default=str)[:1000]
    suffix = f" [{status}]" if status else ""
    return f"Created project `{pid}` — {pname}{suffix}"
...
def list_projects() -> str:
    ...
    lines = ["**Projects:**"]
    for p in (result if isinstance(result, list) else [result])[:50]:
        pid = p.get("id") or p.get("projectId") or "?"
        pname = p.get("name") or pid
        status = p.get("status", "")
        suffix = f" [{status}]" if status else ""
        lines.append(f"- **{pname}** (`{pid}`){suffix}")
    return "\n".join(lines)
...
@mcp.tool()
def get_product_machine_plan(project_id: str) -> str:
    ...
    """
    def _do():
        return _get_client().get_product_machine_plan(project_id)
    return json.dumps(_call_with_retry(_do), indent=2, default=str)[:8000]


@mcp.tool()
def update_product_machine_plan(project_id: str, plan: str) -> str:
    ...
    """
    try:
        parsed = json.loads(plan) if plan and plan.strip() else {}
    except json.JSONDecodeError:
        return f"Error: 'plan' must be valid JSON, got: {plan[:200]}"
    if not isinstance(parsed, dict):
        return "Error: 'plan' must be a JSON object."

    def _do():
        return _get_client().update_product_machine_plan(project_id, parsed)
    return json.dumps(_call_with_retry(_do), indent=2, default=str)[:5000]


# ── Runtime Domain Actions (dynamic-agent factory) ───────────────────
#
# Author -> approve -> dispatch loop for custom agents/actions.
...
def register_runtime_action(
    ...
    """
    try:
        parsed_spec = json.loads(agent_spec) if agent_spec and agent_spec.strip() not in ("", "{}") else {}
    except json.JSONDecodeError:
        return f"Error: 'agent_spec' must be valid JSON, got: {agent_spec[:200]}"
    try:
        parsed_components = json.loads(component_ids) if component_ids and component_ids.strip() not in ("", "[]") else []
    except json.JSONDecodeError:
        return f"Error: 'component_ids' must be valid JSON, got: {component_ids[:200]}"
    if not isinstance(parsed_components, list):
        return "Error: 'component_ids' must be a JSON array."

    def _do():
        return _get_client().register_runtime_action(
            domain,
            action,
            description=description,
            agent_spec=parsed_spec or None,
            component_ids=parsed_components or None,
    ...
    status = result.get("status", "")
    return (
        f"Registered runtime action `{result.get('domain', domain)}.{result.get('action', action)}` "
        f"(id=`{rid}`, status={status}). Approve it with approve_runtime_action to make it dispatchable."
        if rid else json.dumps(result, default=str)[:1000]
    )


@mcp.tool()
def approve_runtime_action(action_id: str) -> str:
    ...
    """
    def _do():
        return _get_client().approve_runtime_action(action_id)
    result = _call_with_retry(_do)
    return f"Approved runtime action `{result.get('id', action_id)}` (status={result.get('status', '')})"


@mcp.tool()
def reject_runtime_action(action_id: str) -> str:
    ...
    """
    def _do():
        return _get_client().reject_runtime_action(action_id)
    result = _call_with_retry(_do)
    return f"Rejected runtime action `{result.get('id', action_id)}` (status={result.get('status', '')})"


@mcp.tool()
def list_runtime_actions(status: str = "active") -> str:
    ...
    """
    def _do():
        return _get_client().list_runtime_actions(status=status)
    result = _call_with_retry(_do)
    actions = result.get("actions", []) if isinstance(result, dict) else (result or [])
    if not actions:
        return f"No runtime actions with status `{status}`."
    lines = [f"**{len(actions)} runtime action(s) [{status}]:**"]
    for a in actions[:50]:
        lines.append(
            f"- `{a.get('domain', '?')}.{a.get('action', '?')}` "
...
def list_aoc_ticks(run_id: str = "", limit: int = 20) -> str:
    ...
    if not cleaned_run_id:
        return "run_id is required: pass the AutoCompany control run id (controlRunId) to list its ticks."

    def _do():
        return _get_client().list_aoc_ticks(controlRunId=cleaned_run_id)
