...
class LightbulbClient:
    ...
    def create_project(
        ...
        *,
        ...
        repo_connection_id: str | None = None,
        product_machine_plan: Dict[str, Any] | None = None,
        ...
        if repo_connection_id:
            payload["repo_connection_id"] = repo_connection_id
        if product_machine_plan:
            payload["product_machine_plan"] = dict(product_machine_plan)
    ...
    def list_projects(self) -> List[Dict[str, Any]]:
        ...
        session = self._get_session()
        resp = session.get(f"{self._base_url}/api/projects", headers=self._headers())
        raise_if_error(resp)
        data = resp.json()
        return data if isinstance(data, list) else data.get("items", data.get("projects", []))
    ...
    def get_product_machine_plan(self, project_id: str) -> Dict[str, Any]:
        """Get a project's product-machine plan / deliverables
        (``GET /api/projects/{id}/product-machine/plan``)."""
        session = self._get_session()
        resp = session.get(
            f"{self._base_url}/api/projects/{project_id}/product-machine/plan",
            headers=self._headers(),
        )
        raise_if_error(resp)
        return resp.json()

    def update_product_machine_plan(self, project_id: str, plan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def memory_store(self, key: str, value: str, *, namespace: str = "default") -> Dict[str, Any]:
        ...
        body: Dict[str, Any] = {
            ...
            "details_json": {"key": key, "namespace": ns},
        }
        session = self._get_session()
        resp = session.post(
    ...
    def memory_search(self, query: str, *, namespace: str = "default", top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        """
        ns = (namespace or "default").strip() or "default"
        body: Dict[str, Any] = {"query_text": query, "top_k": top_k}
        if ns:
            body["categories"] = [ns[:64]]
        session = self._get_session()
        resp = session.post(
    ...
    #
    # Author -> approve -> dispatch loop for custom agents/actions. The whole
    # surface is gated behind the backend flag ``agents.runtime-registration.enabled``
    # (default false); when off these endpoints return 403.
    ...
    def register_runtime_action(
        ...
        *,
        ...
        """
        payload: Dict[str, Any] = {"domain": domain, "action": action}
        if description:
            payload["description"] = description
        if agent_spec is not None:
            payload["agentSpec"] = dict(agent_spec)
        if component_ids is not None:
            payload["componentIds"] = list(component_ids)
        if workflow_type:
    ...
    def approve_runtime_action(self, action_id: str) -> Dict[str, Any]:
    ...
    def reject_runtime_action(self, action_id: str) -> Dict[str, Any]:
    ...
    def list_runtime_actions(self, status: str = "active") -> Dict[str, Any]:
